...
def getmessages():
    ...
    try:
        if usr and 'message' in usr:
            ...
            chat_name = usr['message']['chat'].get(
                'first_name', '') + " " + usr['message']['chat'].get('last_name', '')
            ...
            if ('caption' in usr['message']):
                ...
                if 'document' in usr['message'] and usr['message']['document']['mime_type'] == 'text/plain' and usr['message']['caption'] == '/input':
                    ...
                    file_path = requests.get(
                        f"https://api.telegram.org/bot{API_KEY}/getFile?file_id={file_id}")
                    file_content = requests.get(
                        f"https://api.telegram.org/file/bot{API_KEY}/{file_path.json()['result']['file_path']}")
                ...
                if usr['message']['text'].startswith('/input'):
                    ...
                    processed, listofrag = form_prompt(
                        userq=txt, k=7, user_id=chat_id)
                    ...
                    if listofrag:
                        ...
                        db.session.execute(text(update_sql_cm), {'current_timestamp': datetime.now(
                            ZoneInfo("Asia/Ho_Chi_Minh")).isoformat()})
