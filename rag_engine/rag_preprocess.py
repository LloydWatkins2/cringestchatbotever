...
def add_text_features(texte, chat_id):
    select_cm = text(
        'select count(*) from chatbot_vector where id = :chat_ide')
    count = db.session.execute(select_cm, {'chat_ide': chat_id}).scalar()
    lm = count-301
    if lm > 0:
        sql_lru_cm = text(
            """delete from chatbot_vector where id in (select id from chatbot_vector where chat_id = :chat_ide order by created_at asc limit :lm)""")
        ...
        try:
            docs.append(
                Document(
                    ...
                    metadata={'source': 'user_file',
                              'author': user_name, 'file_name': i.name}
                )
            )
...
embedmodel = OpenAIEmbeddings(
    # Specify the OpenRouter model slug
...
def embed_docs(docs, chat_id):
    ...
    sql_command = text(
        """insert into chatbot_vector (content,embedding,user_id,created_at) values(:content,:embedding,:user_id,:time) """)
    for i in range(len(data)):
        db.session.execute(sql_command, {'content': doclist[i], 'embedding': str(
            data[i]), 'user_id': chat_id, 'time': datetime.now().isoformat()})
...
def response(userq, k, user_id):
    ...
    sql_command = text(
        """select content,1-(embedding<=> cast(:vu as vector)) as cosine_diff,id from chatbot_vector where user_id = :user_id order by embedding <=> cast(:vu as vector) limit :limit_k""")
    req = db.session.execute(sql_command, {"vu": str(
        embedqur), 'user_id': user_id, "limit_k": k})
    # print(req.fetchall())
    return req.fetchall()
